...
上述代码中可以看到一个 ``EOF`` 符号，这个符号称之为 (end of file，文件结束)
。代表的是一个输入的结束符，上述中为什么使用 ``int`` 的一个重要的原因就是这个 c
要有足够空间存储所有的可以的字符包括 ``EOF``，而它定义在 ``<stdio.h>`` 中，并且是一个整型数。

上述的代码可以将为c赋值的操作放在while循环语句的测试部分中

[1_5_2.c](../chapter_one/1_5_2.c)
